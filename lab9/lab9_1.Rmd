...
Columns and Observations not in a single row.

3. Use `pivot_longer()` to make the data tidy. Focus the data only on genotype, water_sched_prog, and greenhouse.  
```{r}
plant_data %>% 
  pivot_longer(-c(genotype, water_sched_prog, greenhouse), #religion will not move
               names_to = "v1", #make a new column called "income"
               values_to="v2", #values moved to a new column called "total"
               values_drop_na = T
               )
```
