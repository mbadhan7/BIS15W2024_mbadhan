...
p+theme_linedraw()+
  ...
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL, #removes label from the x axis
       y= "n",
       fill= "Trophic Guild")
```
...
```{r}
display.brewer.pal(3,"PuBuGn") #sequential palette
```
...
```{r}
q+scale_colour_brewer(palette = "YlOrBr")
```
...
```{r}
my_palette <- paletteer_d("ggsci::default_igv") #store your palette
```
