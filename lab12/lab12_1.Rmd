...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram(color = "black", fill = "mediumorchid2", bins=10)+
  labs(title = "Distribution of Body Mass")
```
...
```{r}
homerange %>% 
  ggplot(aes(x = log10.hra)) + #only specify x
  geom_histogram(color = "black", fill = "mediumorchid2", alpha=0.3)+
...
homerange %>% 
  ...
  geom_density(fill="mediumorchid2", alpha  =0.4, color = "black")+ #alpha is the transparency
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x=log10.hra)) +
  geom_histogram(aes(y = after_stat(density)), fill = "mediumorchid2", alpha = 0.4, color = "black")+
  geom_density(color = "red")+
  labs(title = "Distribution of Body Mass")
```
...
```{r}
homerange %>%
  select(log10.mass) %>% #continuous variable 
